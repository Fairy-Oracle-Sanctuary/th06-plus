...
ZunResult Player::AddedCallback(Player *p)
{
    ...
    // Initialize hitbox indicator
    p->hitboxIndicator.Initialize();
    g_AnmManager->LoadTexture(100, "data/hitbox.png", 1, 0); // TEX_FMT_A8R8G8B8 = 1
    p->hitboxSprite.sourceFileIndex = 100;
    p->hitboxSprite.startPixelInclusive.x = 0.0f;
    p->hitboxSprite.startPixelInclusive.y = 0.0f;
    p->hitboxSprite.endPixelInclusive.x = 64.0f;
    p->hitboxSprite.endPixelInclusive.y = 64.0f;
    p->hitboxSprite.textureWidth = 64.0f;
    p->hitboxSprite.textureHeight = 64.0f;
    p->hitboxSprite.uvStart.x = 0.0f;
    p->hitboxSprite.uvStart.y = 0.0f;
    p->hitboxSprite.uvEnd.x = 1.0f;
    p->hitboxSprite.uvEnd.y = 1.0f;
    p->hitboxSprite.heightPx = 64.0f;
    p->hitboxSprite.widthPx = 64.0f;
    p->hitboxSprite.spriteId = 0;
    p->hitboxIndicator.sprite = &p->hitboxSprite;
    p->hitboxIndicator.activeSpriteIndex = 0;
    p->hitboxIndicator.color = COLOR_SET_ALPHA(COLOR_WHITE, 0);
    p->hitboxIndicator.pos.z = 0.5f;
    p->hitboxAlpha = 0;
    p->hitboxTargetAlpha = 0;
...
ChainCallbackResult Player::OnUpdate(Player *p)
{
    ...
    // Update hitbox indicator alpha (fade in/out)
    p->hitboxTargetAlpha = p->isFocus ? 255 : 0;
    {
        int curAlpha = (int)p->hitboxAlpha;
        int targetAlpha = (int)p->hitboxTargetAlpha;
        ...
        p->hitboxAlpha = (u8)curAlpha;
    }

    if (p->hitboxAlpha > 0)
    {
        p->hitboxIndicator.rotation.z -= 0.02f * g_Supervisor.effectiveFramerateMultiplier;
    }
    p->hitboxIndicator.color = COLOR_SET_ALPHA(COLOR_WHITE, p->hitboxAlpha);
...
ChainCallbackResult Player::OnDrawHighPrio(Player *p)
{
    ...
    if (!g_GameManager.isInRetryMenu)
    {
        ...
        // Draw hitbox indicator when visible
        if (p->hitboxAlpha > 0)
        {
            p->hitboxIndicator.pos.x = g_GameManager.arcadeRegionTopLeftPos.x + p->positionCenter.x;
            p->hitboxIndicator.pos.y = g_GameManager.arcadeRegionTopLeftPos.y + p->positionCenter.y;
            p->hitboxIndicator.pos.z = 0.48f;
            g_AnmManager->Draw(&p->hitboxIndicator);
        }
    }
    return CHAIN_CALLBACK_RESULT_CONTINUE;
}
