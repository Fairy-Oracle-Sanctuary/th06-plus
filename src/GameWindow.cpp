...
RenderResult GameWindow::Render()
{
    ...
    if (g_Supervisor.cfg.windowed == false && !g_Supervisor.ShouldRunAt60Fps())
    {
        // Fix for 1000fps bug: Add software frame rate limiting as fallback when vsync fails
        if (this->curFrame != 0)
        {
            g_Supervisor.framerateMultiplier = 1.0;
            timeBeginPeriod(1);
            ...
            if (local_34 >= FRAME_TIME)
            {
                // Frame took long enough, catch up and continue
                do
                {
                    g_LastFrameTime += FRAME_TIME;
                    local_34 -= FRAME_TIME;
            ...
            {
                // Frame was too fast, wait for remaining time (vsync fallback)
